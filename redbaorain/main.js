(function (window) {
    ...
    var weddingPresent = function (name, options, callback) {
        ...
        // 是否开启调试面板
        this.debuggerPanel = options.debuggerPanel;
        // 标记红包是否下完了
        this.noHb = false;
    };
    weddingPresent.prototype = {
        ...
        init: function () {
            ...
            this.ctx.canvas.addEventListener('touch', this.handler(this));
            //初始化红包
        ...
        concatWedInfo: function (data, status) {
            ...
            if (status === 0) {
                ...
                for (var i = 0; i < data.length; ++i) {
                    ...
                    arr[i] = {
                        id: i,
                        x: ((this.WIDTH) * random + (this.direction === 'top' ? 0 : this.WIDTH / 3)),
        ...
        animate: function () {
            return function () {
                ...
                var hb = this.wPresent;
                var fps = this.calculateFps(new Date());
                this.draw(this.ctx, hb, fps);
                if ((+new Date - this.timer <= this.delayTime * 1000) && !this.noHb) {
        ...
        changeFps: function (fps) {
            if (this.debuggerPanel) {
                var hbFps = util.getId('hbFps');
                hbFps.textContent = parseInt(fps, 10);
            }
        },
        //清空画布
        clear: function () {
        ...
        //设置ctx
        setCtx: function () {
            return util.createCanvasContext2D(this.NAME);
        },
        //获取画布宽高
        getExtent: function () {
            return {
        ...
        destroy: function () {
            ...
            this.ctx.canvas.removeEventListener('touch', this.handler(this));
        }
    ...
    var panel = function () {
        ...
        str += '<p>fps:<span id="hbFps">0</span></p>';
        str += '</div>';
